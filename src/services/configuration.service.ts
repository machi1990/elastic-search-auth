...
export class ConfigurationService {
    ...
    public constructor(@inject(ElasticSearchService) private esService: ElasticSearchService, @inject(Logger) private logger: Logger) {
   ...
   private async prepareMapping() {
    ...
    private async updateVersion() {
        try {
           const updated = await this.esService.client
           .update({
             index: this.INDEX,
             type: this.TYPE,
           ...
           this.logger.info(JSON.stringify(updated, null, 2));
            ...
            const res = await this.esService.client
              .create({
                index: this.INDEX,
                type: this.TYPE,
                id: this.ES_AUTH_VERSION.key,
    ...
    public async get(key) {
       try {
        const config = await this.esService.client
        .get({
          index: this.INDEX,
          type: this.TYPE,
          id: key,
    ...
    public async delete(key) {
       try {
          const deleted = await this.esService.client
          .delete({
            index: this.INDEX,
            type: this.TYPE,
            id: key,
    ...
    public async list(query: Object) {
        query = query || {};
        try {
            const resp = await this.esService.client
            .search({
              index: this.INDEX,
              type: this.TYPE,
              from: query['from'] || 0,
